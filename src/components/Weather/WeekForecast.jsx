...
const WeekForecast = () => {
  ...
  const searchLocation = (e) => {
    fetch(
      `${api.staticURL}onecall?lat=${latitude}&lon=${longitude}&exclude=current,minutely,hourly,alerts&units=metric&appid=${api.key2}`
    )
      .then((res) => res.json())
      .then((result) => {
        setWeather2(result);
  ...
  return (
    <div className="weatherContainer">
      <div className="searchContainer">
        <div>
          <h3 className="header">Enter coordinates:</h3>
          <input
            className="input"
            type="text"
